      ...
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="mb-8 text-sm">
          <ol className="flex items-center gap-2 text-foreground/60">
            ...
            <li>/</li>
            <li><Link href="/sequences" className="hover:text-accent">Sequences</Link></li>
            <li>/</li>
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {sequence.tags.map(tag => (
                <span key={tag} className="px-3 py-1 bg-surface rounded-full text-xs text-foreground/60 border border-border">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div>
            {/* Header */}
            <div className="mb-6">
              <p className="text-accent text-sm font-medium mb-2">Light of Elm Ridge</p>
            ...
            <div className="bg-surface-light rounded-xl p-4 mb-6 border border-border">
              <p className="text-sm text-foreground/70">
                <span className="font-semibold text-accent">Currently available on xlightsseq.com</span> — This sequence contains programming for the props listed below. Purchase allows sequence data to be used in a single residential non-commercial display.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-surface rounded-lg p-3 text-center border border-border">
                <div className="text-lg font-bold text-foreground">{sequence.duration}</div>
                <div className="text-xs text-foreground/50">Duration</div>
              </div>
              <div className="bg-surface rounded-lg p-3 text-center border border-border">
                <div className="text-lg font-bold text-foreground">{sequence.difficulty}</div>
                <div className="text-xs text-foreground/50">Difficulty</div>
              </div>
              <div className="bg-surface rounded-lg p-3 text-center border border-border">
                <div className="text-lg font-bold text-foreground">{sequence.propCount}+</div>
            ...
            <div className="mb-6 p-4 bg-surface rounded-xl border border-border">
              <p className="text-sm text-foreground/60">
                <span className="font-semibold text-foreground">Purchase the audio:</span> {sequence.audioSource}
              </p>
            </div>
          ...
          <div className="bg-surface rounded-xl p-6 border border-border">
            ...
            <div className="flex flex-wrap gap-2">
              {sequence.models.map(model => (
                <span key={model} className="px-3 py-1 bg-surface-light rounded-lg text-sm text-foreground/70 border border-border">
                  {model}
                </span>
              ))}
            </div>
        ...
        <div className="bg-surface rounded-xl p-8 border border-border mb-16">
          ...
          <div className="prose prose-invert max-w-none">
            {sequence.longDescription.split('\n\n').map((paragraph, i) => (
              <p key={i} className="text-foreground/70 mb-4 whitespace-pre-line">
                {paragraph}
              </p>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-8 border-t border-border">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-foreground/50">Get started immediately</p>
          </div>
          <div className="text-center">
            <div className="text-2xl mb-2">💬</div>
            <h3 className="font-semibold text-sm">Support</h3>
            <p className="text-xs text-foreground/50">We&apos;re here to help</p>
          </div>
          <div className="text-center">
            <div className="text-2xl mb-2">✅</div>
            <h3 className="font-semibold text-sm">Tested & Ready</h3>
            <p className="text-xs text-foreground/50">Runs on real displays</p>
          </div>
        </div>
